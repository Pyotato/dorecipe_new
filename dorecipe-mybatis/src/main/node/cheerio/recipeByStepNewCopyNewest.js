  ...
  for (let items2 in cat2) {
    for (let items3 in cat3) {
      for (let items4 in cat4) {
        // console.log("cat1 ", cat1[items]);
        // console.log("cat2 ", cat2[items2]);
        // console.log("cat3 ", cat3[items3]);
        // console.log("cat4 ", cat4[items4]);
        ...
        // if (links.length % 40 == 0) {
        //   onCategoryChangeIndex.push(links.length);
        // }
      }
    }
  }
...
for (let i = 0; i < onCategoryChangeIndex.length; i++) {
  console.log(
    ...
    // recipeLinkList.indexOf(onCategoryChangeIndex[i]) 바뀌는 링크의 인덱스
  );
}
